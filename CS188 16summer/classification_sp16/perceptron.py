...
class PerceptronClassifier(object):
    ...
    def train(self, input_train_data, label_train_data, input_val_data, label_val_data, iterations, callback=None):
        ...
        for iteration in range(iterations):
            ...
            for i in range(len(input_train_data)):
                ...
                "*** YOUR CODE HERE ***"
                #print 'training_data:',input_train_data[i]
                #print 'label_data:',label_train_data[i]
                #print 'classified',self.classify(input_train_data[i])
                #classify training set first
                classified = self.classify(input_train_data[i])
                #y' = y*
                if classified != label_train_data[i]:
                    "wy* = wy* + f"
                    "wy' = wy' - f"
    ...
    def find_high_weight_features(self, label):
        ...
        "*** YOUR CODE HERE ***"
        import operator
        #print "label",self.legal_labels
        #print label
        #print self.weights
        sortedweight = sorted(self.weights[label].iteritems(), key=operator.itemgetter(1), reverse=True)  

        for i in range(100):
            #print "weight:",sortedweight[i][0]
            best100Features.append(sortedweight[i][0])
