...
class GradientDescentSolver(Solver):
    ...
    def get_updates_with_momentum(self, loss_tensor, param_vars):
        ...
        #util.raiseNotDefined()
        for i in range(len(param_vars)):
            next_vel = self.momentum * vel_vars[i] - self.learning_rate * grad_tensors[i]
            updates.append((param_vars[i], param_vars[i] + next_vel))
            updates.append((vel_vars[i], next_vel))
        return updates
    ...
    def solve(self, input_train_data, target_train_data, input_val_data, target_val_data, model, callback=None):
        ...
        for iter_ in range(self.iterations):
            ...
            for i in range(len(update_ops)):
                train, updates = session.run([loss_tensor, update_ops[i]], feed_dict={model.input_ph:input_train_data, target_ph: target_train_data})
                train_loss += train
                val = session.run(loss_tensor, feed_dict={model.input_ph:input_val_data, target_ph: target_val_data})
                val_loss += val
